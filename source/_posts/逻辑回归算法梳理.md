...
$$
\begin{aligned}
\log{\frac{P(y=1|x,\theta)}{P(y=K|x,\theta)} } &= \theta_1^T x \tag{39} \\
\log{\frac{P(y=2|x,\theta)}{P(y=K|x,\theta)} } &= \theta_2^T x \tag{40} \\
&\vdots \\
\log{\frac{P(y=K-1|x,\theta)}{P(y=K|x,\theta)} } &= \theta_{K-1}^T x \tag{41}
\end{aligned}
$$
...
$$
P(y=k|x,\theta) = \begin{cases}
\dfrac{e^{\theta_k^T x}}{1+\sum_{t=1}^{K-1}e^{\theta_t^T x}} & k=1,2,...,K-1 \\[1em]
\dfrac{1}{1+\sum_{t=1}^{K-1}e^{\theta_t^T x}} & k=K
\end{cases}
\tag{43}
$$
