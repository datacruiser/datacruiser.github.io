...
- 再举个例子说明上述两点的优化，假设数据集$A$的某个特征的特征值有（二叉树存储）：{% raw %}$\{1.2,1.3,2.2,2.3,3.1,3.3\}${% endraw %}，预排序算法要全部遍历一遍，需要切分大约5次。进行离散化后，只需要切分2次 {% raw %}$\{\{1\},\{2,3\}\}${% endraw %} 和 {% raw %}$\{\{1,2\},\{3\}\}${% endraw %}，除了切分次数减少，内存消耗也大大降低。
...
{% raw %}$$\hat{x}_k^i=\frac{\sum_{j=1}^n I_{\{x_j^i=x_k^i\}}*y_j+a\,P}{\sum_{j=1}^n I_{\{x_j^i=x_k^i\}}+a}$${% endraw %}
